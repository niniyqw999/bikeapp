...
const untils_distance = require("../../untils/distance.js");
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onMounted(() => {
      untils_request.getTrack({ phoneNumber: user.userInfo.phoneNumber }).then((res) => {
    ...
    const del = (id) => {
      untils_request.delTrack(id).then((res) => {
        if (res.data.code === 200) {
          untils_request.getTrack({ phoneNumber: user.userInfo.phoneNumber }).then((res2) => {
    ...
    return (_ctx, _cache) => {
      return {
        a: common_vendor.f(common_vendor.unref(trackList), (item, index, i0) => {
          return {
            a: common_vendor.t(common_vendor.unref(untils_distance.getDistance)(item.tracks[0].latitude, item.tracks[0].longitude, item.tracks[item.tracks.length - 1].latitude, item.tracks[item.tracks.length - 1].longitude).toFixed(2)),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "D:/wxapp/bikeApp/myRegion/pages/track/track.vue"]]);
